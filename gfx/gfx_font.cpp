...
gfx_font::gfx_font( uint16_t y_advance ):
	v_reference_count( true ),
	_x_advance( 0 ),
...
gfx_font* gfx_font::make_gfx_font_from_glyphmap( const uint8_t* glyphmap, uint8_t first_glyph, uint8_t last_glyph, gfx_xy_t width, gfx_xy_t height, bool lsb_first, bool invert ) {
	...
	gfx_font_ref font = new gfx_font();
	font->_y_advance = 8;
...
gfx_font* gfx_font::make_gfx_font_from_arduino( const arduino_gfx_font_data* arduinofont ) {
	...
	gfx_font_ref font = new gfx_font( arduinofont->width + 1 );
...
gfx_font* gfx_font::make_gfx_font_from_adafruit( const adafruit_gfx_font_data* adafont ) {
	...
	gfx_font_ref font = new gfx_font( adafont->yAdvance );

	uint16_t glyph_count = adafont->last - adafont->first;
...
void gfx_font::print_gfx_font_files( gfx_font_ref font, const char* font_name ) {
...
void gfx_font::print_gfx_font_array( gfx_font_ref font, const char* font_name ) {
...
void gfx_font::print_gfx_font_asciiart( gfx_font_ref font, const char* font_name ) {
	uint bit_count = 0;
	for ( const auto& [ character, glyph ] : font->get_glyph_map() ) {
