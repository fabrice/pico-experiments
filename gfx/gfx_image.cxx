...
gfx_bytemap_data gfx_image::make_yx_bytemap() const {
	const gfx_dimension dimension = this->get_dimension();
	const gfx_wh_t dybytes = (dimension.get_height() + 7) / 8;
	gfx_bytemap_data bytemap( dimension.get_width() * dybytes, 0 );
...
std::vector< uint16_t > gfx_image::make_pixmap_565() const {
	...
	for ( gfx_xy_t y = 0 ; y < dimension.get_height() ; ++ y ) {
		for ( gfx_xy_t x = 0 ; x < dimension.get_width() ; ++ x ) {
			...
			pixmap[ x + y * dimension.get_width() ] = static_cast< uint16_t >((color & 0x00ff) << 8) | static_cast< uint16_t >((color & 0xff00) >> 8);
		}
	}
